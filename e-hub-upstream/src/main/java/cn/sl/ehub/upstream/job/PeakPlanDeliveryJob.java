...
/**
 * 调峰计划申报96点数据电网上送Job（基础用电上报+可调能力上报）
 *
...
@Slf4j
@Component
public class PeakPlanDeliveryJob {
    ...
    public void execute() {
        ...
        if (StringUtils.isBlank(aggregatorId)) {
            log.warn("调峰计划申报96点数据电网上送服务-未配置aggregatorId，跳过执行");
            return;
        }
        ...
        jobThreadPool.execute(() -> {
            try {
                ...
                String lockKey = String.format("VPP:PEAK_PLAN:96POINT:DELIVERY:LOCK:%s:%s", aggregatorId, now);
                RedisLock lock = new RedisLock(redisTemplate, lockKey);
                log.info("调峰计划申报96点数据电网上送服务：lockKey:{}", lockKey);

                if (lock.lockV3()) {
                    log.info("调峰计划申报96点数据电网上送服务-获取定时任务锁成功");
                    // aggregatorId = 聚合商ID
                    boolean result = peakPlanDeliveryService.execute96PointDelivery(aggregatorId);
                    log.info("调峰计划申报96点数据电网上送服务-执行结果：{}", result ? "成功" : "失败");
                } else {
                    log.info("调峰计划申报96点数据电网上送服务-未取到定时任务锁，已有其他节点执行");
                }
            } catch (Exception e) {
                log.error("调峰计划申报96点数据电网上送服务-执行异常：{}", ExceptionUtils.getStackTrace(e));
            }
        });
    }
}
