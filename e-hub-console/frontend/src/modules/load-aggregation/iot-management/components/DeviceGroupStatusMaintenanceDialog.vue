...
<script>
import {
  createDeviceGroupPointDefinition,
  deleteDeviceGroupPointDefinition,
  listDeviceGroupPointDefinitions,
  updateDeviceGroupPointDefinition,
} from "../api/index.js";

export default {
  name: "DeviceGroupStatusMaintenanceDialog",
  props: {
    visible: {
      type: Boolean,
      default: false,
  ...
  methods: {
    ...
    async removeRow(row) {
      ...
      const res = await deleteDeviceGroupPointDefinition(row.id);
      const body = res.data || {};
      if (body.code === 200) {
