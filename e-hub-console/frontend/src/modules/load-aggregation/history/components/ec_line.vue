    ...
    opt() {
      ...
      const opt = {
          ...
          formatter(p) {
            if (that.tooltip_show) {
              ...
              p.forEach((item, index) => {
                let showUnit = that.unit;
                that.ecdata.forEach(ec => {
                  if (item.seriesName === ec.name) {
                    if (ec.yAxisIndex === 0) {
                      showUnit = that.unit;
                    } else if (ec.yAxisIndex === 1) {
                      showUnit = that.unit2;
                    }
                  }
                });
        ...
        grid: {
          ...
          left: this.isScreen ? 75 : 45,
          // grid 组件离容器右侧的距离。默认10%
          right: this.isScreen ? 30 : that.unit2 ? 45 : 20,
