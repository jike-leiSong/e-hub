...
export default {
  ...
  props: {
    ...
    color:{
      type:Array,
      default:()=>["#42A642", "#0780ED", "#00AFC3", "#B06EF3", "#68C4FD", "#FF5227", "#AE716E", "#0050E2", "#F6D159", "#911FA8"]
    },
    isScreen: {
      type: Boolean,
      default: false
    ...
    opt() {
      ...
      let opt = {
          ...
          formatter(p) {
            if (that.tooltip_show) {
              ...
              p.forEach((item, index) => {
                ...
                y_name += `<p style="min-width: 150px;display:flex;justify-content:space-between;align-items:center"><span>${item.marker}${item.seriesName}：</span>${
        ...
        legend: {
          // selectedMode: 'single',
          type: 'scroll',
          show: that.origin[0] && that.origin[0].name? true: false,
          // left:80,
          // 图例标记的图形宽度。默认25px
          itemWidth: 6,
          // 图例标记的图形高度。默认14px
          ...
          data: that.origin ? that.origin.map((item, index) => {
            return { name: item.name, icon: 'rect', textStyle: { color: item.color ? item.color : that.color } }
          }) : []
        },
        dataZoom: [
          // 滑动条型数据区域缩放组件
        ...
        series: that.origin ? that.origin.map((item, index) => {
          return {
            ...
            data: item.value && item.value.length ? item.value.map((item, index) => {
              if (item.quantity === 0 || item.value === 0 || item.dateValue === 0 || item.useQuantity === 0) {
                return 0
              } else {
